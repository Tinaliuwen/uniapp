		...
		<view class="commodity-info">
			...
			<!-- <view class="info-intro"> -->
				...
				<!-- <view class="type">
					<view>
						...
						<text v-else>{{ dataList.price_express }}元</text>
					</view>
				</view> -->
			<!-- </view> -->
			...
			<view class="info-detail-evaluate" :style="{ height: height + 'px' }">
				...
				<swiper :current="tabIndex" class="swiper-box" duration="300"  @change="changeTab">
					...
					<!-- <swiper-item class="tab-content" v-show="false">
						<scroll-view class="list-scroll-content" scroll-y>
							<view class="evaluate" v-if="goodComment.length != 0">
								<view class="title">用户评价 （{{ groudList.comment_num }}）</view>
								<view class="user-info" v-for="(item, index) in goodComment" :key="index">
									<view class="photo">
									...
									<view class="info">
										...
										<view class="img">
											<view v-for="(item, index) in imgList.images" :key="index">
												<image :src="item" mode=""></image>
											</view>
										</view>

										<view class="merchant">{{ item.reply_content }}</view>
										<view class="img">
											<view v-for="(item, index) in imgList.reply_images" :key="index">
												<image :src="item" mode=""></image>
											</view>
										</view>
										<view class="add-comment">用户{{ item.count }}天后追评</view>
										<view class="comment-list">
											{{ item.append_content }}
											<view class="img">
												<view class="img" v-for="(item, index) in imgList.append_images" :key="index">
													<image :src="item" mode=""></image>
												</view>
											</view>
										</view>
										<view class="merchant">
											...
											<view class="img" style="margin-bottom: 100upx;">
												<view v-for="(item, index) in imgList.append_reply_images" :key="index">
													<image :src="item" mode=""></image>
												</view>
											</view>
										</view>
							...
							<view class="not-data" v-else>暂无评价</view>
						</scroll-view>
					</swiper-item> -->
				</swiper>
			</view>
			...
			<view class="btn"  v-if="stock > 0">
				<view class="to">
					...
					<view class="home" >
						<view class="" v-if="is_collection==0" @click="_collection(dataList.id)">
						...
						<view class="" style="color: #ff594d;" v-else>已收藏</view>
					</view>
				</view>
				<view class="conversion" style="background-color: rgba(255,89,77,.4);color: #ff594d;" @click="toggleSpec(1)" v-if="btntype == 1">加入购物车</view>
				<!-- <view class="conversion" @click="toggleSpec(2)":class="this.auction == 0 ? 'auction':''" >立即拍卖</view> -->
				<view class="conversion"  @click.stop="tell()">立即购买</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			getGoodsDetail() {
				var self = this;
				self.$http
					.post(service.api.category.goodDetail, {
						goodsid: self.gid,
			...
			tell(){
				var self = this;
				self.$http
					.post(service.api.order.getPaymeslo).then(function(res) {
						if(res.code == 1){
