<template>
	<view class="review">
		<!-- 已取消 -->
		<view class="state" v-if="status == 0"></view>
		<!-- 待付款 -->
		<view v-if="status == 2">
			<view class="state">
				<view>
					<view class="success">待付款</view>
					<view class="total-price">订单金额：{{ orderDetail.pay_price }} 元</view>
				</view>
			...
			<view v-for="(item, index) in orderDetail.allGoods">
				<view class="order-center" @click="jump(orderDetail.goodsid)">
					<view class="img"><image class="order-img" :src="item.goods_logo" mode=""></image></view>
					<view class="center-right">
						<view class="title">{{ item.goods_title }}</view>
						<view class="detail">
							<view class="detail-price">￥{{ item.price_selling }}</view>
							<view class="detail-num">x{{ item.number_goods }}</view>
						</view>
					</view>
				</view>
				<view class="content-center">
					<view class="price">
						...
						<view class="price-text">￥{{ item.price_selling }}</view>
					</view>
					<view class="price" style="margin: 10upx 0;">
						<view class="price-text">运费</view>
						<view class="price-text">￥{{ item.price_express }}</view>
					</view>
					...
					<view class="total">
						...
						<view class="total-price-right">￥{{ orderDetail.pay_price }}</view>
					</view>
				</view>
			</view>
			<view class="order-bottom">
				<view class="time" style="color: #333;">订单编号： {{ orderDetail.order_no }}</view>
				<view class="time" style="color: #333;">创建时间： {{ orderDetail.create_at }}</view>
				<view class="bottom">
 ...
 -->
		...
		<!-- 	<view class="order-center" @click="jump(orderDetail.goodsid)">
				...
				<view class="center-right">
					...
					<view class="detail">
						<view class="detail-price">￥{{ orderDetail.pay_price }}</view>
			...
			<view class="content-center">
				<view class="price">
					<view class="price-text">订单总价（含运费）</view>
					<view class="price-text">￥{{ orderDetail.pay_price }}</view>
				</view>
				<view class="price" style="margin: 10upx 0;">
					<view class="price-text">运费</view>
		...
		<view v-if="status == 5">
			...
			<view v-for="(item, index) in orderDetail.allGoods">
				<view class="order-center" @click="jump(orderDetail.goodsid)">
					<view class="img"><image class="order-img" mode=""></image></view>
					<view class="center-right">
						<view class="title">{{ item.goods_title }}</view>
						<view class="detail">
							<view class="detail-price">￥{{ orderDetail.pay_price }}</view>
		...
		getOrderDetail() {
			...
			self.$http
				...
				.then(function(res) {
					...
					self.address = list;
					self.address.addressList = list.express_province + list.express_city + list.express_area + list.express_address;
					self.address.username =list.username  
					self.address.mobile =list.mobile 
...
.state {
	...
	background: linear-gradient(90deg, #e0252a 35%, #f66669 100%);
	height: 206upx;
	display: flex;
