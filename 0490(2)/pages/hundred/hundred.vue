<template>
	<view class="hundred">
		<view class="navbar">
			<view class="nav-box">
				<view v-for="(item, index) in navList" :key="index" class="nav-item" :class="{ current: tabIndex === index }" @click="tabClick(index)">{{ item.name }}</view>
			</view>
		</view>
		...
		<swiper :current="tabIndex" class="swiper-box" duration="300" @change="changeTab">
				...
				<scroll-view class="list-scroll-content" scroll-y>
					...
					<view class="swiper-section">
						<swiper :indicator-dots="true" :autoplay="false" :interval="3000" :duration="1000" indicator-active-color="#fff">
							...
							<swiper-item>
								<view class="swiper-item"><image src="/static/img/points/banner.png" mode=""></image></view>
							</swiper-item> -->
						</swiper>
					</view>
					
					<view class="type">
						<view class="synthesize active">综合</view>
						<view class="count">销量</view>
						<view class="price">
							价格
							<image src="/static/img/updown.png" mode=""></image>
						</view>
						<view class="screen" @click="screening = !screening">
							筛选
							<image src="/static/img/screen.png" mode=""></image>
						</view>
					</view>

					<view class="list">
						<view class="product" v-for="(item, index) in groudList" :key="index" @click="jump('/pages/hundred/commodity?goodsid=' + item.id, item.id)">
			...
			<!-- 平价包邮 -->
			<swiper-item class="tab-content"><scroll-view class="list-scroll-content" scroll-y></scroll-view></swiper-item>
			<!-- 今日特惠 -->
			<swiper-item class="tab-content"><scroll-view class="list-scroll-content" scroll-y></scroll-view></swiper-item>
			<!-- 健康杂粮 -->
			<swiper-item class="tab-content"><scroll-view class="list-scroll-content" scroll-y></scroll-view></swiper-item>
		...
		<view class="screen-box" v-if="screening" @click.self="screening = !screening">
			<view class="box">
				...
				<view class="classify-type">
					<view class="classify-box" v-for="(item, index) in navList" :key="index">{{ item.text }}</view>
				</view>
				<view class="price title">价格区间（元）</view>
				<view class="section">
					<view class="">最低价</view>
					<view class="">最高价</view>
				</view>
...
export default {
	...
	onLoad: function(e) {
		...
		var self = this;
		self.$http.get(service.api.group.categroy).then(function(res) {
			// console.log(res.data.categroy);

			var list = res.data;
			self.navList = list;
			self.getSwiper();
			self.tabClick(Number(e.type));
	...
	methods: {
		...
		getGroupList() {
			var self = this;
			self.cateid = self.navList[self.tabIndex].id;
			/* console.log(this.navList[this.tabIndex].id);
			console.log(self.cateid,555); */
			self.$http
				.get(service.api.group.goodsList, {
					cateid: self.cateid
				})
				.then(function(res) {
					// console.log(res.data.categroy);
